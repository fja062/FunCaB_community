...
### Functional group richness (Graminoids only)
```{r}
...
# Load the fg_richness_G_analysis target
tar_load(fg_richness_G_analysis)

# Check model diagnostics for the 2-way model
result <- check_model_diagnostics(fg_richness_G_analysis$model_2way)
result  # This will plot the diagnostics
```

### Functional group identity (Graminoids only)
```{r}
#| label: fig-fg-identity-G-diagnostics
#| echo: false
#| message: false
#| fig-height: 12

# Load the fg_identity_G_analysis target
tar_load(fg_identity_G_analysis)

# Check model diagnostics for the 2-way model
result <- check_model_diagnostics(fg_identity_G_analysis$model_2way)
result  # This will plot the diagnostics
``` 
